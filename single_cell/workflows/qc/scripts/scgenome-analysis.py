...
logging.info(scgenome.__file__)

from scgenome.loaders.qc import load_qc_data
import scgenome.loaders.snv

dir = snakemake.params["tantalus"]
ticket = snakemake.params["ticket"]
sample_id = snakemake.params["sample_id"]
library_id = snakemake.params["library_id"]
snv_genotyping_ticket = snakemake.params["snv_genotyping_ticket"]

print(dir, ticket, sample_id, library_id, snv_genotyping_ticket)
print("\n\n\n")
print(snakemake.params)

logging.info(snakemake.params)
...
print(dir + snv_genotyping_ticket,"\b\b\b\b",  snv_data[['chrom', 'coord', 'ref', 'alt']].drop_duplicates(),
...
prefix = snakemake.params["prefix"] + "_"
...
fig.savefig(prefix + 'mutations_per_cell.png', bbox_inches='tight')
...
high_impact.to_csv(prefix + 'snvs_high_impact.csv')
...
snv_data.to_csv(prefix + "snvs_all.csv")
...
tri_nuc_table = (snv_data_sig.groupby(['num_cells_class', 'tri_nuc_idx'])
                ...
                .rename(columns={0: "n"}))
tri_nuc_table.to_csv(prefix + 'trinuc.csv')
...
fig.savefig(prefix + 'snv_adjacent_distance.png', bbox_inches='tight')

# Plot snv count as a histogram across the genome
fig = plt.figure(figsize=(10, 3))
wgs_analysis.plots.snv.snv_count_plot(plt.gca(), snv_data)
fig.savefig(prefix + 'snv_genome_count.png', bbox_inches='tight')
...
fig.savefig(prefix + 'snv_cell_counts.png', bbox_inches='tight')
...
fig.savefig(prefix + 'snv_alt_counts.png', bbox_inches='tight')
...
if len(breakpoint_data.sample_id) == 0:
    ...
    fig.savefig(prefix + 'rearranegementtype_distribution.png', bbox_inches='tight')
    #fig.savefig(prefix + 'breakpoint_adjacent_distance.png', bbox_inches='tight')
    fig.savefig(prefix + 'chromosome_types.png', bbox_inches='tight')
    ...
    #     ax, breakends)
    # ax.set_title(f'Breakend adjacent distances')
    # plt.tight_layout()
    # fig.savefig(prefix + 'breakpoint_adjacent_distance.png', bbox_inches='tight')
...
fig.savefig(prefix + 'BAFplot.png', bbox_inches='tight')
...
if len(cn_data_filt.chr) == 0:
    fig = plt.figure(figsize=(8, 12))
    fig.savefig(prefix + 'CNplot.png', bbox_inches='tight')
else:
    cn_data_summary = (
...
df = pd.DataFrame(d)
df.to_csv(prefix + 'datatype_summary.csv')
