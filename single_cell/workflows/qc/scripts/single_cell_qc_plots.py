...
def load_snv_data(sample_id, library_id, prefix, mappability_file, strelka_file, 
...
def load_breakpoint_data(sample_id, library_id, breakpoint_annotation, breakpoint_count, 
    lumpy=False, filter=False):
...
def plot_breakpoint_distribution(sample_id, breakpoint_data, 
    rearranegementtype_distribution, chromosome_types, is_lumpy=False
):
    features = [
    ...
    if is_lumpy:
        breakpoint_data = breakpoint_data.rename(columns={'breakpoint_id':"prediction_id",
            'chrom1': "chromosome_1", 'strand1': "strand_1", 'start1':"position_1",
...
def load_qc_data(sample_id, annotation_metrics, hmmcopy_reads, hmmcopy_segs, hmmcopy_metrics, 
...
def qc_plots(sample_id,  mappability_file, strelka_file, 
    ...
    metrics_data, cn_data_filt = load_qc_data(sample_id, annotation_metrics, hmmcopy_reads, 
    ...
    #######################################
    d = {'datatype': ['snv', 'cnv', 'haplotypes'], 'ncells': [len(snv_count_data.cell_id.unique()), 
            len(metrics_data["cell_id"].unique()), len(allele_data["cell_id"].unique())]
        }
    df = pd.DataFrame(d)
