...
class ExtractHmmMetrics(object):
    def __init__(self, params, reads, segments, output, sample_id):
        self.params = params
    ...
    def compute_quality_metrics(self, df, df_seg, sample_id):    
        if 'copy' in df.columns:
            ...
            if df_seg is not None:
                MBRSM_dispersion = self.median_of_bin_residuals_from_segment_median(df, df_seg)
                
                MSRSI_non_integerness = self.median_of_segment_residuals_from_segment_integer(df_seg)
    ...
    def main(self):
        ...
        df_metrics.to_csv(self.output, index=False)
