...
docker run -e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -e AWS_DEFAULT_REGION -v $PWD:$PWD -w $PWD $1/awscli:v0.0.1 \
  aws s3 cp s3://singlecelltestsets/TESTDATA_CODEBUILD/breakpoint-calling BREAKPOINT_CALLING/ref_test_data --recursive --quiet

docker run -w $PWD -v $PWD:$PWD -v /refdata:/refdata --rm \
  $1/single_cell_pipeline_breakpoint:$TAG \
  single_cell breakpoint_calling \
  --input_yaml single_cell/tests/codebuild/breakpoint_calling/inputs.yaml \
  --maxjobs $NUMCORES \
  --nocleanup \
  --sentinel_only \
  --submit local \
...
docker run -w $PWD -v $PWD:$PWD -v /refdata:/refdata --rm \
  $1/single_cell_pipeline_breakpoint:$TAG \
  python single_cell/tests/codebuild/breakpoint_calling/test_breakpoint_calling.py BREAKPOINT_CALLING/output BREAKPOINT_CALLING/ref_test_data/refdata
