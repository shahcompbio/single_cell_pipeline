...
docker run -e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -e AWS_DEFAULT_REGION -v $PWD:$PWD -w $PWD $1/awscli:v0.0.1 \
  aws s3 cp s3://singlecelltestsets/TESTDATA_CODEBUILD/pseudo_bulk_qc PSEUDO_BULK_QC/ref_test_data --recursive --quiet

docker run -w $PWD -v $PWD:$PWD -v /refdata:/refdata --rm \
  $1/single_cell_pipeline_qc:$TAG \
  single_cell pseudo_bulk_qc --input_yaml single_cell/tests/codebuild/pseudo_bulk_qc/inputs.yaml \
